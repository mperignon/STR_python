...
int GetData(double printmatrix[][101], double Slmatrix[][101], double qbmatrix[][101],
    double Hmatrix[][101], double x[], int M, int prints) {
    ...
    while (loop == 1) {
        skip = 0;
        printi=400005;
        printf("Specify the nondimensional time of the printout.\n");
        scanf("%f", &time);
        for(i=0; i<=prints; i++) {
        ...
        if (skip == 0) {
            ...
            fprintf(ptrOut2, "\n\teta\tSl\tqbT\tH\n");
            if (time >= 100) {
                fprintf(ptrOut2, "x\t%.0lfyr\t%.0lfyr\t%.0lfyr\t%.0lfyr\n", printmatrix[printi][0], Slmatrix[printi][0], qbmatrix[printi][0], Hmatrix[printi][0]);
            }
            else if (time >= 0.1) {
                fprintf(ptrOut2, "x\t%.2lfyr\t%.2lfyr\t%.2lfyr\t%.2lfyr\n", printmatrix[printi][0], Slmatrix[printi][0], qbmatrix[printi][0], Hmatrix[printi][0]);
            }
            else {
                fprintf(ptrOut2, "x\t%.5lfyr\t%.5lfyr\t%.5lfyr\t%.5lfyr\n", printmatrix[printi][0], Slmatrix[printi][0], qbmatrix[printi][0], Hmatrix[printi][0]);
            }
            for (i=1; i <= (M+1); i++) {
                fprintf(ptrOut2, "%lf\t%lf\t%lf\t%lf\t%lf\n", x[i], printmatrix[printi][i], Slmatrix[printi][i], qbmatrix[printi][i], Hmatrix[printi][i]);
            }
            fclose(ptrOut2);
        }
