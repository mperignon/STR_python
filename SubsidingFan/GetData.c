...
int GetData(double printmatrix[][101], double Slmatrix[][101], double qbmatrix[][101],
    ...
    while (loop == 1) {
        ...
        if (printi == 40005) {
            printf("There is no printout for that time.  ");
        ...
        if (skip == 0) {
            ...
            fprintf(ptrOut2, "\n\teta(m)\tSl\tqbT\n");
            if (time >= 100) {
                fprintf(ptrOut2, "x(m)\t%.0lfyr\t%.0lfyr\t%.0lfyr\n", printmatrix[printi][0], Slmatrix[printi][0], qbmatrix[printi][0]);
            }
            else if (time >= 0.1) {
                fprintf(ptrOut2, "x(m)\t%.2lfyr\t%.2lfyr\t%.2lfyr\n", printmatrix[printi][0], Slmatrix[printi][0], qbmatrix[printi][0]);
            }
            else {
                fprintf(ptrOut2, "x(m)\t%.5lfyr\t%.5lfyr\t%.5lfyr\n", printmatrix[printi][0], Slmatrix[printi][0], qbmatrix[printi][0]);
            }
            for (i=1; i <= (M+1); i++) {
                fprintf(ptrOut2, "%lf\t%lf\t%lf\t%lf\n", x[i], printmatrix[printi][i], Slmatrix[printi][i], qbmatrix[printi][i]);
            }
            fclose(ptrOut2);
        }
