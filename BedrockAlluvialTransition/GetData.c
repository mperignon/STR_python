...
int GetData(double printmatrix[][101], double Slmatrix[][101], double qbmatrix[][101],
    double xmatrix[][101], double Hmatrix[][101], int M, int prints) {
    ...
    while (loop == 1) {
        ...
        if (skip == 0) {
            ...
            fprintf(ptrOut2, "\nx(m)\teta(m)\tSl\tqb\tH(m)\n");
            if (time >= 100) {
                fprintf(ptrOut2, "%.0lfyr\t%.0lfyr\t%.0lfyr\t%.0lfyr\t%.0lfyr\n", xmatrix[printi][0], printmatrix[printi][0], Slmatrix[printi][0], qbmatrix[printi][0],  Hmatrix[printi][0]);
            }
            else if (time >= 0.1) {
                fprintf(ptrOut2, "%.2lfyr\t%.2lfyr\t%.2lfyr\t%.2lfyr\t%.2lfyr\n", xmatrix[printi][0], printmatrix[printi][0], Slmatrix[printi][0], qbmatrix[printi][0],  Hmatrix[printi][0]);
            }
            else {
                fprintf(ptrOut2, "%.5lfyr\t%.5lfyr\t%.5lfyr\t%.5lfyr\t%.5lfyr\n", xmatrix[printi][0], printmatrix[printi][0], Slmatrix[printi][0], qbmatrix[printi][0],  Hmatrix[printi][0]);
            }
            for (i=1; i <= (M+1); i++) {
                fprintf(ptrOut2, "%lf\t%lf\t%lf\t%lf\t%lf\n", xmatrix[printi][i], printmatrix[printi][i], Slmatrix[printi][i], qbmatrix[printi][i], Hmatrix[printi][i]);
            }
            fclose(ptrOut2);
        }
