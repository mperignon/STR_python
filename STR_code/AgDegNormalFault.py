...
class AgDegNormalFault(object):
    ...
    def __init__(self,
        ...
        self.fault_position = float(fault_position)
        self.fault_height = float(fault_height)
        self.fault_time = float(fault_time)
        self.faulted = False
    ...
    def fault_profile(self):
        """
        Set self.faulted to True once the fault moves
        """
        self.faulted = True
        
        ind_fault = int(round(self.fault_position * len(self.eta))) -1
        
        self.eta[ind_fault:] = self.eta[ind_fault:] - self.fault_height
    ...
    def run(self):
        ...
        for t in np.arange(0, self.totalT+self.dt, self.dt):
            ...
            if t >= self.fault_time and not self.faulted:
                self.fault_profile()
    ...
    def finalize(self):
        ...
        if self.save_output:
            ...
            for i in range(len(self.fields)):
                ...
                np.savetxt('output/AgDegNormalFault_' + self.fields[i] + '.csv', self.data,
                               header = header,
                               delimiter = ",",
                               fmt = '%10.8f',
                               comments = '')