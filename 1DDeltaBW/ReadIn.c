...
int ReadIn(double *qw, double *I, double *qtf, double *D, double *Cz, double *nt,
    ...
    double *dt, double *dxbar, double *Cf, double *sfmax, double *ksid, int *M,
    int *prints, int *iterates, int *formulation) {
    ...
    if (*formulation > 2) {
        ...
        if ((*formulation != 1) && (*formulation != 2)) {
            printf("Improper response, please start over and try again...\n");
            return 1;
        }
