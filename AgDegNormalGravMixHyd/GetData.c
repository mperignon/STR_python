...
int GetData(double printmatrix[][101], double Slmatrix[][101], double Hmatrix[][101],
    double taumatrix[][101], double qbmatrix[][101], double dsgmatrix[][101], double D90matrix[][101], 
    double x[], int M, int prints, double qbToave) {
    //Initialize
    ...
    while (loop == 1) {
        ...
        if (skip == 0) {
            ...
            for (i=1; i <= (M+1); i++) {
                fprintf(ptrOut2, "%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", x[i], printmatrix[printi][i], Slmatrix[printi][i], Hmatrix[printi][i], taumatrix[printi][i], (qbmatrix[printi][i]*qbToave), dsgmatrix[printi][i], D90matrix[printi][i]);
            }
            fclose(ptrOut2);
        }
