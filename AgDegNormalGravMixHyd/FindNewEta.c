...
int FindNewEta(quad GSD[], int M, int npp, int np, double na, double Dx90s[],  
               double qbT[], double Fs[], double Ft[], double plf[], double F[][16],
               double pl[][16], double eta[], double alphau, double qbTf, double dx,
               double atrans, double dt, double I, double lps, int *counter, double dsgs[],
               double Laold[]) {
    //Initialize
    double La[M+2], Flexc[M+2][npp+2], qjj1dev[npp+2], qjj2dev[npp+2], etanew[M+2];
    double Fnew[M+2][16], Fft[npp+2];
    double qbTdev=0, dsgst=0, Dx90st=0;
    int i=0, j=0;
    ...
    for (i=1; i <= (M+1); i++) {
        ...
        dsgs[i] = dsgst;
        Perfiner(Ft, Fft, npp);
        Interpolation(GSD, Fft, npp, &Dx90st);
        Dx90s[i] = Dx90st;
        for (j=1; j <= np; j++) {
            F[i][j] = Ft[j];
        }
