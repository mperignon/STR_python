...
int GetData(double printmatrix[][101], double Slmatrix[][101], double Hmatrix[][101],
    double taumatrix[][101], double qbmatrix[][101], double ksimatrix[][101], double x[],
    int M, int prints) {
    //Initialize
    ...
    while (loop == 1) {
        ...
        if (skip == 0) {
            ...
            fprintf(ptrOut2, "\n\teta(m)\tSl\tH(m)\tksi(m)\ttau\tqb\n");
            if (time >= 100) {
                fprintf(ptrOut2, "x(m)\t%.0lfyr\t%.0lfyr\t%.0lfyr\t%.0lfyr\t%.0lfyr\t%.0lfyr\n", printmatrix[printi][0], Slmatrix[printi][0], Hmatrix[printi][0], ksimatrix[printi][0], taumatrix[printi][0], qbmatrix[printi][0]);
            }
            else if (time >= 0.1) {
            ...
            for (i=1; i <= (M+1); i++) {
                fprintf(ptrOut2, "%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", x[i], printmatrix[printi][i], Slmatrix[printi][i], Hmatrix[printi][i], ksimatrix[printi][i], taumatrix[printi][i], qbmatrix[printi][i]);
            }
            fclose(ptrOut2);
        }
