...
int FindNewEta(quad GSD[], int M, int npp, int np, double na, double Dx90s[], double Dx50s[], 
               ...
               double fracsand[], double Laold[], double subrate, double ExnerFactor) {
    //Initialize
    ...
    for(i=1; i<= (M+1); i++) {
        ...
        etanew[i] = eta[i] - subrate*dt + dt*(-qbTdev*I*ExnerFactor/(1.0 - lps));
        for (j=1; j <= np; j++) {
            Fnew[i][j] = F[i][j] + dt*I*ExnerFactor*(-qjj1dev[j] + qjj2dev[j])/(La[i]*(1.0-lps));
            if (time > 0) {
