...
int Initialize(quad GSD[], double *qw, double *qbTf, double *I, double *etad, double *S,
    ...
    double fracsand[], double *dx, double *Sinu, double *subrate, double *lamda, double *rB,
    double *ExnerFactor, double *Cf, int *formulation) {
    ...
    check = ReadIn(GSD, &(*qw), &(*qbTf), &(*I), &(*etad), &(*S), &(*L),
                    ...
                    &(*alphau), &(*atrans), &(*npp), &(*np), &(*rload), &(*Cexp), &(*nexp), &(*Sinu),
                    &(*subrate), &(*lamda), &(*rB), &(*ExnerFactor), &(*Cf), &(*formulation));
    if (check == 1)
        return 1;
