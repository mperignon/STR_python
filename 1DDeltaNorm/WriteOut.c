...
int WriteOut(double printmatrix[][102], double xmatrix[][102], double suvector[],
    double sssvector[], double sbbvector[], double Volinit, double Volfin, double Volfed,
    double ervol, int prints, int M) {
    ...
    fprintf(ptrOut, "\n");
    fprintf(ptrOut, "time(yr)\tsu\tsss\tsbb\tetaup\tetatop\tetabottom\n");
    for (i=0; i <= prints; i++) {
        fprintf(ptrOut, "%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", printmatrix[i][0], suvector[i], sssvector[i], sbbvector[i], printmatrix[i][1], printmatrix[i][M+1], printmatrix[i][M+2]);
    }
    fprintf(ptrOut, "\n");
